...
const webEntry = {};
const weexEntry = {};
const HotPackage = require('./hotpackage')
...
const getNativeEntryFileContent = (entryPath, vueFilePath) => {
  let relativeVuePath = path.relative(path.join(entryPath, '../'), vueFilePath);
  let relativeEntryPath = helper.root(config.nativeEntryFilePath);
  let contents = fs.readFileSync(relativeEntryPath, {
    encoding: "utf-8"
  });
  if (isWin) {
    relativeVuePath = relativeVuePath.replace(/\\/g, '\\\\');
  }
  contents = contents.replace(/VUE_RELATIVE_PATH/g, relativeVuePath)
...
const newWeexEntry = HotPackage.getPackageFiles(weexEntry)

console.log('要打包的weex路径', JSON.stringify(newWeexEntry))
...
// PACKAGE_ENV:打包环境  DEV:开发环境 DT:测试环境 GREY:灰度环境 IDC:生产环境
const PACKAGE_ENV = process.env.PACKAGE_ENV;

console.log("当前环境为:", PACKAGE_ENV);
